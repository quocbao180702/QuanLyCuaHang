...
namespace DAO
{
    public class NhanVien_DAO
    {
        ...
        public static List<NhanVien_DTO> ThongTinCaNhan(string ma)
        {
            string str = string.Format("select * from nhanvien where manv = '{0}'",ma);
            conn = DataProvider.MoKetNoi();
            DataTable dt = DataProvider.TruyVanLayDuLieu(str, conn);
            if (dt.Rows.Count == 0)
            {
        ...
        {
            string sTruyVan = string.Format(@"insert into nhanvien values(N'{0}',N'{1}',N'{2}',N'{3}','{4}',N'{5}',N'{6}',N'{7}',N'{8}',N'{9}',N'{10}')", nv.SMaNV, nv.SHoLot, nv.STen, nv.SPhai, nv.DtNgaySinh,nv.SSdt, nv.SEmail,nv.SDiaChi,nv.STinh, nv.SMaCV, nv.SMaCH);
            conn = DataProvider.MoKetNoi();
        ...
        {
            string sTruyVan = string.Format(@"select * from nhanvien where holot like N'%{0}%' and tennv like N'%{1}%'", ho, ten);
            conn = DataProvider.MoKetNoi();
            DataTable dt = DataProvider.TruyVanLayDuLieu(sTruyVan, conn);
            if (dt.Rows.Count == 0)
            {
        ...
        {
            string sTruyVan = string.Format(@"update nhanvien set manv = '{0}', holot = N'{1}', tennv = N'{2}',gioitinh = N'{3}',ngaysinh = '{4}',sdt = N'{5}',email = '{6}',diachi = N'{7}',tinh = N'{8}',macv = '{9}',mach = '{10}' where manv = '{0}'", nv.SMaNV, nv.SHoLot, nv.STen, nv.SPhai, nv.DtNgaySinh,nv.SSdt, nv.SEmail, nv.SDiaChi, nv.STinh, nv.SMaCV, nv.SMaCH, nv.SMaCV);
            conn = DataProvider.MoKetNoi();
            bool kq = DataProvider.TruyVanKhongLayDuLieu(sTruyVan, conn);
            DataProvider.DongKetNoi(conn);
            return kq;
        ...
        {
            string str = string.Format(@"Delete_NhanVien '{0}'", ma);
            conn = DataProvider.MoKetNoi();
        ...
        {
            string sTruyVan = string.Format(@"select * from nhanvien where holot like N'%{0}%'", ho);
            conn = DataProvider.MoKetNoi();
            DataTable dt = DataProvider.TruyVanLayDuLieu(sTruyVan, conn);
            if (dt.Rows.Count == 0)
            {
